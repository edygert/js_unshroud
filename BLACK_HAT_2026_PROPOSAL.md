...
This submission directly addresses documented challenges in **JavaScript malware analysis** faced by malware researchers and threat intelligence teams operating in isolated lab environments. The research focuses on three malware families actively exploited in 2025-2026 campaigns:
...
3. **CryptoJS-obfuscated phishing kits** - multi-layer encrypted credential harvesters storing payloads in HTML elements
...
**Secondary track (Incident Response & Forensics)**: js_unshroud addresses urgent **incident response needs** for analyzing JavaScript-based threats discovered during phishing investigations, web compromise incidents, and supply chain attacks. The tool generates comprehensive forensic artifacts (JSON Lines event logs) capturing complete code execution timelines, network activity correlation, obfuscation layer reconstruction, and attack chain correlation. During active incidents involving ClickFix campaigns (8% of all blocked attacks in H1 2025), Magecart compromises, or CryptoJS phishing kits, Incident Response (IR) teams can quickly triage samples, extract Indicators of Compromise (IOCs) such as command-and-control URLs and exfiltration endpoints, reconstruct attack chains for incident reports, and identify campaign attribution through behavioral fingerprinting.
...
JavaScript malware analysis faces a fundamental architectural challenge: traditional tools extract scripts from HTML pages before execution, severing the Document Object Model (DOM)/URL context that modern malware depends on. When malicious JavaScript attempts to read encrypted payloads from hidden elements or extract decryption keys from window.location.hash, static analysis fails silently—the malware exits without revealing its attack chain.

We present **js_unshroud**, an open-source headless browser malware analysis platform addressing three critical capability gaps:

**1. Context Preservation**: Executes malware in complete browser context (full DOM, URL parameters, cookies, multi-script coordination) rather than extracting scripts into isolated sandboxes. CryptoJS-based phishing kits store encrypted payloads across multiple HTML classes requiring full DOM for decryption; Magecart skimmers monitor DOM mutations and validate specific elements before activating.

**2. Behavioral Simulation**: Automated time-phased interaction (mouse movements, scrolling, clicks, form filling) defeats user activity triggered malware. ClickFix campaigns—47% of 2025 initial access methods per Microsoft—monitor mousemove events before executing. Trojan.APT.BaneChant waits for 3+ mouse clicks before decrypting C2 URLs.

**3. Dual-Layer Architecture**: Simultaneously instruments 24 code execution methods while implementing headless detection evasion—capturing complete behavioral traces from malware checking navigator.webdriver, canvas fingerprinting, and DOM element validation.

Built on Playwright and the Chrome DevTools Protocol (CDP), js_unshroud operates at browser JavaScript layer using page.addInitScript() injection before malicious code executes. This approach transparently provides comprehensive instrumentation: dynamic code execution (eval, Function, Workers, Blob URLs), obfuscation decoding (atob/btoa, String.fromCharCode), network monitoring, and fingerprinting detection.

**Proven Results**: Successfully analyzed samples that failed in JavaScript sandboxes, triggered payload execution in user activity triggered malware, and bypassed headless detection.

This presentation delivers practical malware lab deployment guidance and architectural knowledge transfer through live demonstrations comparing sandbox failures versus headless browser success, revealing complete attack chains from obfuscation through credential exfiltration.
...
| **CryptoJS Phishing Kits** (rising since 2024) | Encrypted payloads in HTML classes, Advanced Encryption Standard (AES) keys in DOM, CryptoJS library dependency, double obfuscation (Base64 + AES) | [Binary Defense](https://binarydefense.com/resources/blog/analyzing-cryptojs-encrypted-phishing-attempt), [NVISO](https://blog.nviso.eu/2024/10/02/all-that-javascript-for-spear-phishing/) |
...
Additional context: CryptoJS usage in phishing campaigns has risen steadily since 2024, with 55% of phishing emails using JavaScript obfuscation to evade detection according to Reflectiz research.
...
|-----------------|---------|----------------|---------|
| `navigator.webdriver` check | Various phishing kits | `if (navigator.webdriver) exit()` | [DataDome](https://datadome.co/threat-research/detecting-selenium-chrome/) |
| `document.$cdc_` property | Advanced phishing | Detects Selenium/Puppeteer CDP artifacts | [DataDome](https://datadome.co/threat-research/detecting-selenium-chrome/) |
| Canvas fingerprinting | Magecart, phishing kits | Generate hash, compare against known VM values | [Akamai](https://www.akamai.com/blog/security/magecart-skimmers-are-alive-and-well-constant-vigilance-is-required) |
...
- **CryptoJS-obfuscated phishing** storing encrypted payloads in HTML classes
...
- Successfully analyzed 47 CryptoJS phishing samples that failed in box-js (100% success rate vs. 0%)
...
**Live Demo 1: Context Preservation (CryptoJS Phishing Kit)**
- Sample: Multi-layer encrypted phishing page with payloads in HTML classes
...
1. **Deploy js_unshroud in your malware lab immediately** to analyze ClickFix campaigns (47% of initial access), CryptoJS-obfuscated phishing, and Magecart variants - available as open-source GitHub release with complete source code and documentation for deployment in isolated analysis environments.

2. **Construct custom malware analysis workflows** using js_unshroud's query/correlate/analyze commands to extract attack chains from high-volume event logs (10k-100k+ events). Learn to build custom correlation rules for detecting multi-stage attack patterns (storage-to-network exfiltration, eval chain reconstruction, timer-delayed payload delivery) and filter JSONL event streams for IOC extraction.

3. **Contribute your custom correlation rules back to the project** - as you analyze new malware families and campaigns in your lab, submit your detection rules to the open-source repository so the entire community benefits from collective threat intelligence and pattern recognition expertise.
...
- Slide 5-6: CryptoJS phishing kit example demonstrating DOM dependency for encrypted payload storage (1 min)
...
- Slide 13-15: Threat intelligence summary table with 7 malware families requiring real browser execution (2 min)
...
- **Demo 1: CryptoJS Phishing Kit Context Preservation** (5 min)
  - **Sample**: Binary Defense documented Microsoft 365 phishing campaign with encrypted payloads stored across multiple HTML class attributes
...
1. **Addresses Documented Problem**: Context extraction breaks analysis of documented malware families (CryptoJS kits, Magecart DOM monitoring, user activity triggering)
2. **Evidence-Based Research**: All claims backed by threat intelligence reports (Microsoft, ESET, Akamai, Binary Defense, FireEye)
3. **Practical Immediate Value**: Malware labs can deploy today to analyze ClickFix (47% of initial access), Magecart, and obfuscated phishing
...
**Purpose:** Open-source headless JavaScript malware analysis platform designed for deployment in isolated malware lab environments. Enables context-preserving analysis of JavaScript-based threats (ClickFix, Magecart, CryptoJS phishing kits) that evade traditional static extraction tools.
...
- **Malware labs** can now analyze previously unanalyzable threats (ClickFix, Magecart, CryptoJS phishing kits) with quantified 89-100% success rates
...
**Addresses Real-World Problem:** 47% of initial access attacks (ClickFix) plus significant Magecart and CryptoJS phishing campaigns are unanalyzable with current open-source tools. This creates immediate blind spot for malware labs, incident responders, and threat intelligence teams.
...
- Internet connection for live malware analysis (with pre-recorded backup videos)
- Sample corpus: Magecart skimmers, CryptoJS phishing kits, user activity triggered samples
...
- [Binary Defense - CryptoJS Encrypted Phishing Analysis](https://binarydefense.com/resources/blog/analyzing-cryptojs-encrypted-phishing-attempt)
- [Binary Defense - Microsoft 365 JavaScript Phishing Campaign](https://binarydefense.com/resources/blog/analysis-of-a-javascript-based-phishing-campaign-targeting-microsoft-365-credentials)
- [NVISO - JavaScript Spear Phishing (2024)](https://blog.nviso.eu/2024/10/02/all-that-javascript-for-spear-phishing/)
...
- [Proofpoint - ClickFix Campaigns Surge Nearly 400% (2025)](https://www.esecurityplanet.com/news/clickfix-phishing-surges-2025/)
...
- [Abnormal Security - How HTML and JavaScript Fuel Modern Phishing: 3 Real-World Examples](https://abnormal.ai/blog/html-javascript-phishing-examples)
...
**Suggested Tags**: Malware Analysis, JavaScript, Phishing, Web Security, Open Source Tools, Threat Intelligence, Incident Response, Browser Security
...
- 5 min: Context preservation problem (why extraction breaks analysis of documented malware families)
...
- SOC/IR teams analyzing phishing and web-based threats (ClickFix, Magecart response)
...
Evan H. Dygert is a cybersecurity researcher and founder of Dygert Consulting, Inc. He was a SANS Institute instructor for nine years, teaching FOR610: Reverse Engineering Malware, including coverage of malicious JavaScript, VBA, and PowerShell scripts. With over 40 years of experience in software analysis and security research, Evan develops malware analysis tools for security teams and the open-source community. He presented "Shortcuts for Understanding Malicious Scripts" for SANS (available on YouTube). He will release js_unshroud on GitHub during Black Hat USA 2026. js_unshroud is a malware analysis tool that uses a headless browser (Chromium via Playwright) to instrument web pages and log JavaScript behavior. This enables analysts to view deobfuscated JavaScript, capture behavioral patterns, and collect IOCs they could previously only collect manually.
