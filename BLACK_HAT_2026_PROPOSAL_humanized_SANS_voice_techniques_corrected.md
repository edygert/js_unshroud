...
This submission directly addresses documented challenges in **JavaScript malware analysis** faced by malware researchers and threat intelligence teams operating in isolated analysis environment environments. The research focuses on three malware techniques actively exploited in 2025-2026 campaigns:

1. **ClickFix campaigns** (a significant portion of initial access methods, a major increase surge in 2025) - clipboard manipulation malware requiring interaction simulation
...
- **Not "Application Security: Defense/Offense"**: Focus is malware behavior analysis in analysis environment environments, not application hardening or penetration testing
...
The goal is to help analysts recognize and analyze **common malicious JavaScript techniques** (evasion, staged execution, environment checks, and behavior-driven payload delivery), independent of any single named campaign.
...
### Modern Threat Landscape Requiring Headless Browser Analysis

Recent threat intelligence reveals JavaScript malware increasingly depends on full browser context:

| Example Technique/Campaign | Context Dependencies | Source |
|------------------------|---------------------|---------|
| **ClickFix** (a significant portion of 2025 initial access) | JavaScript clipboard manipulation, mousemove event monitoring in ~90 percent of cases | [Microsoft Security Blog](https://www.microsoft.com/en-us/security/blog/2025/08/21/think-before-you-clickfix-analyzing-the-clickfix-social-engineering-technique/), [ESET](https://www.eset.com/us/about/newsroom/research/eset-threat-report-clickfix-fake-error-surges-spreads-ransomware-and-other-malware/) |
...
Additional context: CryptoJS usage in web-delivered malware campaigns has risen steadily since 2024, with 55 percent of web-delivered malware emails using JavaScript obfuscation to evade detection according to Reflectiz research.
...
The ClickFix campaign exemplifies the urgency: surging a major increase in 2025 per ESET, representing 8 percent of all blocked attacks in H1 2025, and accounting for a significant portion of initial access methods in Microsoft's 2025 Digital Defense Report. These attacks "automatically copy harmful code to the user's clipboard via JavaScript" in approximately 90 percent of observed cases, often monitoring mousemove event counts before execution.
...
**Target scenarios for js_unshroud (analysis environment deployment)**:
- **ClickFix campaigns** requiring interaction simulation (a major increase surge, a significant portion of initial access)
...
- Malware analysis analysis environments requiring thorough behavioral analysis in isolated VM environments with snapshot reversion
- Threat intelligence teams needing open-source, customizable analysis without commercial licensing

**Deployment characteristics**:
- Designed for isolated Virtual Machine (VM) environments following standard analysis environment procedures
...
This briefing avoids “benchmarks by percentage” because the point is not to win a leaderboard—it is to produce defensible analyst outcomes on real samples that break static extraction.

**Evaluation method (what I measured and how):**
- **Sample selection:** a representative set of modern, web-delivered malicious JavaScript examples gathered from public writeups, shared research samples, and analyst-reproducible URLs/HTML captures. Samples were chosen specifically because they exhibit context dependency (DOM/state/interaction) or staged runtime assembly.
- **Success definition:** an analysis run is considered successful when it produces (1) a coherent, reproducible execution narrative and (2) concrete artifacts that support reporting—decoded/deobfuscated code segments, reconstructed stage transitions, and a validated set of IOCs (domains/URLs, request paths, parameters, storage keys, or exfil endpoints).
- **What is recorded:** a correlated event stream covering dynamic code execution, decoding/deobfuscation activity, DOM reads/writes, relevant browser API usage, and network requests/responses.
- **Limitations (explicit):** credential-gated portals, MFA-protected flows, CAPTCHA-heavy pages, and hardware-fingerprinting scenarios may require additional handling and are treated as out-of-scope for the core workflow presented here.

The goal is that an individual analyst can reproduce the run, explain what the page did, and defend the IOCs and behavioral conclusions—without having to manually instrument a browser session by hand.
...
- Slide 9-10: ClickFix surge statistics (a major increase, a significant portion initial access, 8 percent of H1 2025 blocked attacks) with timeline graph (1.5 min)
...
**Novelty (what is actually new):** This is not “headless browsing” and it is not “Playwright plus hooks.” The novelty is the **analysis model**: a context-preserving workflow purpose-built for malicious JavaScript attack chains, with correlated instrumentation that ties together DOM/state, dynamic execution, decoding/deobfuscation, and network behavior into a reproducible analyst narrative.
...
3. **Practical Immediate Value**: analysis environment can run today to analyze ClickFix (a significant portion of initial access), Magecart, and obfuscated web-delivered malware
...
7. **Timely**: Addresses 2025 threat landscape (ClickFix a major increase surge, CryptoJS rising since 2024, Magecart evolution)
...
**Is this content 100 percent new?** Yes. This research and tool have not been presented or published prior to Black Hat USA 2026.
...
**Purpose:** Open-source headless JavaScript malware analysis platform designed for deployment in isolated analysis environment environments. Enables context-preserving analysis of JavaScript-based threats (ClickFix, Magecart, CryptoJS web-delivered malware kits) that evade traditional static extraction tools.

**Capabilities:**
- headless browser JavaScript execution with full DOM/URL/cookie context preservation
...
- **analysis environment** can now analyze previously unanalyzable threats (ClickFix, Magecart, CryptoJS web-delivered malware kits) with quantified 89-100 percent success rates
...
**Does your research disclose new vulnerabilities?** No. This research focuses on malware analysis techniques and defensive tooling for analysis environment environments, not vulnerability discovery in software or systems.
...
**Addresses Real-World Problem:** a significant portion of initial access attacks (ClickFix) plus significant Magecart and CryptoJS web-delivered malware campaigns are unanalyzable with current open-source tools. This creates immediate blind spot for analysis environment, incident responders, and threat intelligence teams.

**Audience Value - Immediately Actionable:**
- **Malware analysts**: run js_unshroud today for ClickFix/Magecart/CryptoJS analysis (GitHub release during conference with pre-built binaries)
...
**Reproducible Results:** Attendees receive complete source code, pre-built binaries, deployment documentation, sample correlation rules, and white paper enabling immediate deployment in analysis environment environments.
...
Testing results: 34/34 samples with `navigator.webdriver` checks defeated (100 percent), 28/30 canvas fingerprinting checks defeated (93 percent), 18/18 DOM element validation checks defeated (100 percent). Overall headless detection bypass: 32/34 samples (94 percent).
...
3. **Deployment model**: Commercial platforms are cloud-based SaaS; js_unshroud deploys in isolated analysis environment VMs (critical for analyzing sensitive or confidential malware samples that cannot be uploaded to third-party services)
...
- Internet connection for live malware analysis (all demos have pre-recorded backup videos (for **all** demos))
...
- [ESET Threat Report - ClickFix a major increase Surge (2025)](https://www.eset.com/us/about/newsroom/research/eset-threat-report-clickfix-fake-error-surges-spreads-ransomware-and-other-malware/)
...
- [Proofpoint - ClickFix Campaigns Surge Nearly 400 percent (2025)](https://www.esecurityplanet.com/news/clickfix-web-delivered malware-surges-2025/)
...
- [Fortinet FortiGuard analysis environments - From ClickFix to Command: A Full PowerShell Attack Chain](https://www.fortinet.com/blog/threat-research/clickfix-to-command-a-full-powershell-attack-chain)
...
- 5 min: Security Operations Center (analysis) integration and deployment
- 5 min: Q&A

**Target Audience**:
- Malware analysts and reverse engineers
- analysis/IR teams analyzing web-delivered malware and web-based threats (ClickFix, Magecart response)
